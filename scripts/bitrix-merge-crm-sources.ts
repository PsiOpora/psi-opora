...
function flattenParams(params: Record<string, unknown>, prefix = ""): [string, string][] {
...
function warnDuplicateCanonicalNames(byName: Map<string, StatusEntry[]>) {
  for (const cfg of Object.values(MERGE_PLAN)) {
    ...
    if (entries && entries.length > 1) {
      console.log(
        `!! В справочнике ${entries.length} записей с именем «${cfg.renameFrom}» — использую только ID=${entries[0].ID}, остальные (${entries.slice(1).map((e) => e.ID).join(", ")}) не будут учтены`,
      );
    }
  }
}
...
async function plan() {
  ...
  for (const [name, entries] of byName) {
    for (const e of entries) {
      console.log(`  ID=${String(e.ID).padEnd(5)} STATUS_ID=${String(e.STATUS_ID).padEnd(20)} NAME=${name}`);
    }
  }
  console.log();
  ...
  for (const [entityName, entityTypeId] of Object.entries(ENTITY_TYPE_IDS)) {
    ...
    console.log(`  ${entityName}: ${supported ? "да" : "НЕТ — будет пропущен"}`);
  }
  console.log();
...
async function collectApplyPlan(byName: Map<string, StatusEntry[]>) {
  ...
  for (const [canonical, cfg] of Object.entries(MERGE_PLAN)) {
    ...
    for (const oldName of cfg.mergeFrom) {
      const entries = byName.get(oldName);
      if (!entries) {
        skipped.push(`${canonical}: не найден дубль «${oldName}» (пропускаю только его)`);
        continue;
      }
    ...
    for (const [entityName, entityTypeId] of Object.entries(ENTITY_TYPE_IDS)) {
      ...
      const items = oldCodes.length ? await findItemsBySource(entityTypeId, oldCodes) : [];
      work.push({ canonical, canonicalId, canonicalCode, entityName, entityTypeId, items: items || [] });
    }
  }
  return { work, skipped };
}
...
async function applyMerge() {
  ...
  for (const w of work) {
    if (w.items.length) {
      console.log(`  ${w.canonical}: ${w.items.length} записей в ${w.entityName} -> sourceId=${w.canonicalCode}`);
  ...
  for (const w of work) {
    ...
    console.log(`${w.canonical} / ${w.entityName}: обновляю ${w.items.length} записей`);
    const cmd: Record<string, [string, Record<string, unknown>]> = {};
    for (const item of w.items) {
      cmd[`u${item.id}`] = [
        "crm.item.update",
        { entityTypeId: w.entityTypeId, id: item.id, fields: { sourceId: w.canonicalCode } },
      ];
    }
  ...
  for (const [canonical, cfg] of Object.entries(MERGE_PLAN)) {
    ...
    try {
      ...
      console.log(`  !! не удалось переименовать: ${e instanceof Error ? e.message : e}`);
    }
  }
...
async function cleanup() {
  ...
  for (const { oldName, id } of toDelete) console.log(`  ${oldName} (ID=${id})`);
  ...
  for (const { oldName, id } of toDelete) {
    ...
    try {
      ...
      console.log(`  !! не удалось удалить: ${e instanceof Error ? e.message : e}`);
    }
  }
}
