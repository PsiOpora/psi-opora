...
**Настройка:**
1. В Метрике для psi-opora.ru создайте цель «JavaScript-событие» с условием
   «содержит» и идентификатором `consultation_booked` (или своим — тогда
   переопределите `YANDEX_METRIKA_CONSULTATION_GOAL`).
2. Получите OAuth-токен с доступом к Метрике и впишите
   `YANDEX_METRIKA_COUNTER_ID`/`YANDEX_METRIKA_OAUTH_TOKEN` в `.env` (см.
   комментарии там же).
3. (Необязательно) заведите в Bitrix пользовательское строковое поле сделки
   для ClientID и укажите его код в `BITRIX_YM_CLIENT_ID_FIELD` — тогда
   ClientID будет виден и в самой карточке сделки, не только в Метрике.
4. На сайте пропишите получение ClientID и приклейку суффикса к ссылкам на
...
4. В Bitrix — убедиться, что сделка создалась и (если настроен
   `BITRIX_YM_CLIENT_ID_FIELD`) в ней есть ClientID.
5. В логах бота — найти `yandex_metrika.goal_sent` с тем же ClientID и
   `dealId` (или `yandex_metrika.upload_failed`/`upload_error`, если что-то
   пошло не так).
6. В Метрике → Отчёты → Конверсии — убедиться, что цель
   `consultation_booked` засчиталась (данные появляются в течение ~2 часов
