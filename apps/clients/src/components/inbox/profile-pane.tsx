...
export function ProfilePane({
	...
	// Устаревшая глубокая ссылка могла указывать на канал, который с тех пор
	// объединили с другим клиентом (см. merge.ts) — переключаем выбранный
	// диалог на актуального канонического клиента.
	const profile = data?.profile;
	useEffect(() => {
		if (!profile) return;
		if (
			profile.canonicalMessenger !== selected.messenger ||
			profile.canonicalUserId !== selected.userId
		) {
			onMerged({
				messenger: profile.canonicalMessenger,
				userId: profile.canonicalUserId,
			});
		}
	}, [profile, selected, onMerged]);
