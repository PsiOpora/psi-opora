...
/** Простые монохромные глифы мессенджеров для бейджа на аватаре. */
function TelegramGlyph({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className}>
      <path d="M9.78 18.65l.28-4.23 7.68-6.92c.34-.31-.07-.46-.52-.19L7.74 13.3 3.64 12c-.88-.27-.89-.88.2-1.31l15.97-6.16c.73-.33 1.43.18 1.15 1.3l-2.72 12.81c-.19.91-.74 1.13-1.5.71l-4.14-3.05-2 1.92c-.24.24-.44.44-.9.44z" />
    </svg>
  );
}
...
function MaxGlyph({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className}>
      <path d="M4 4h3.2l4.8 6.4L16.8 4H20v16h-3.2v-10.8l-4.8 6.4-4.8-6.4V20H4z" />
    </svg>
  );
}
...
export function MessengerIcon({
  ...
  switch (messenger) {
    ...
    case "telegram-personal":
      return <TelegramGlyph className={cn(className)} />;
    case "whatsapp-personal":
      return <WhatsAppGlyph className={cn(className)} />;
    case "max":
      return <MaxGlyph className={cn(className)} />;
    default:
      return null;
  }
}
