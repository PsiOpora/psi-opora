...
export async function listCosts(): Promise<CostEntry[]> {
  ...
  if (!hash) return [];
  return Object.values(hash).sort((a, b) => b.month.localeCompare(a.month) || a.utmSource.localeCompare(b.utmSource));
}

export async function addCost(entry: Omit<CostEntry, "id" | "createdAt">): Promise<void> {
  const redis = getRedisOrNull();
  if (!redis) return;
  const id = crypto.randomUUID();
  await redis.hset(COSTS_KEY, { [id]: { ...entry, id, createdAt: new Date().toISOString() } });
}
...
export function monthIntersectsRange(month: string, from: Date, to: Date): boolean {
