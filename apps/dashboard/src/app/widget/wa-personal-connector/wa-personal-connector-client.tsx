...
export function WaPersonalConnectorClient({
	...
	const codeExpired =
		step === "pairing" && secondsLeft <= 0 && codeExpiresAt !== null;
		...
		<div className="flex flex-col gap-1.5">
			...
			{step === "pairing" && (
				<>
					{qr && (
						<div className="flex overflow-hidden rounded-md border text-xs">
							<button
								...
								className={`flex-1 py-1 ${method === "qr" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
							>
								QR-код
							</button>
							<button
								...
								className={`flex-1 py-1 ${method === "code" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
							>
								Код
							</button>
						</div>
					...
					{method === "qr" && qr ? (
						<>
							{/* biome-ignore lint/performance/noImgElement: data-URL из WAHA, next/image тут не подходит */}
							<img
								src={`data:${qr.mimetype};base64,${qr.data}`}
								alt="QR-код для входа в WhatsApp"
								className="mx-auto size-28 rounded"
							/>
					...
					{codeExpired ? (
						<p className="text-xs text-destructive">
							{method === "qr" ? "QR-код" : "Код"}, скорее всего, истёк —
							WhatsApp принимает его недолго.
						</p>
					) : (
						<p className="text-xs text-muted-foreground">
							Действует ещё {minutes}:{seconds}
						</p>
					)}
					<Button
						size="sm"
