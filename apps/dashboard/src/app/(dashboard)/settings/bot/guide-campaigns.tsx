...
function buildAutoTexts(title: string): Record<AutoTextField, string> {
	...
	return {
		...
		emailSubject: trimmed
			? `${quoted} — материалы от Психологического центра «Опора»`
			: "Материалы от Психологического центра «Опора»",
		emailBody:
			"Здравствуйте!\n\n" +
		...
		followUpMessage:
			...
			"диагностическую консультацию с психологом Психологического центра «Опора». " +
...
function viewerMessenger(messenger: string): string {
	if (messenger === "telegram") return "Telegram";
	if (messenger === "max") return "MAX";
	return messenger;
}

const viewerDateFormat = new Intl.DateTimeFormat("ru-RU", {
	dateStyle: "short",
	timeStyle: "short",
});

/**
 * Кто открыл материал: список из bot_guide_views с контактами из выдачи.
 * Запрос уходит только при открытии диалога — на странице настроек кампаний
 * может быть много, и тянуть просмотры по всем сразу не нужно.
 */
function CampaignViewersDialog({
	campaignId,
	title,
	opened,
}: {
	...
	const { data: viewers = [], isLoading } = useQuery({
		...orpc.guideCampaigns.viewers.queryOptions({ input: { campaignId } }),
		enabled: open,
	});
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className="max-w-lg">
				<DialogHeader>
					<DialogTitle>Открыли материал «{title}»</DialogTitle>
					<DialogDescription>
						Переходы по персональной ссылке из чата бота. Материал, ушедший
						вложением в письме или файлом в MAX, отследить нельзя — там нет
						ссылки, по которой можно засечь открытие.
					</DialogDescription>
				</DialogHeader>
				<div className="flex max-h-[60vh] flex-col gap-2 overflow-y-auto">
					...
					{!isLoading && viewers.length === 0 && (
						<span className="text-sm text-muted-foreground">
							Пока никто не открывал ссылку на материал.
						</span>
					)}
					{viewers.map((viewer) => (
						<div
							key={`${viewer.messenger}:${viewer.userId}`}
							className="flex flex-wrap items-baseline justify-between gap-2 rounded border p-2"
						>
							<div className="flex min-w-0 flex-col">
								<span className="truncate text-sm font-medium">
									{viewer.name ||
										`${viewerMessenger(viewer.messenger)} #${viewer.userId}`}
								</span>
								<span className="truncate text-xs text-muted-foreground">
									{[viewer.email, viewer.phone].filter(Boolean).join(" · ") ||
										viewerMessenger(viewer.messenger)}
								</span>
							</div>
							<span className="text-xs text-muted-foreground">
								{viewerDateFormat.format(new Date(viewer.lastOpenedAt))}
								{viewer.opens > 1 && ` · открытий ${viewer.opens}`}
							</span>
						</div>
					))}
				</div>
										...
										<div className="flex min-w-0 flex-1 flex-col gap-1">
											...
											<span className="text-xs text-muted-foreground">
												...
												{campaign.followUpDelayDays} дн. · {(() => {
													...
													return `выдано ${row.delivered} · открыли ${row.opened} · напомнили ${row.followUpSent} · заявок на диагностику ${row.diagnosticRequested}`;
												})()}
											</span>
											{(statsById.get(campaign.id)?.opened ?? 0) > 0 && (
												<CampaignViewersDialog
													campaignId={campaign.id}
													title={campaign.title}
													opened={statsById.get(campaign.id)?.opened ?? 0}
												/>
											)}
