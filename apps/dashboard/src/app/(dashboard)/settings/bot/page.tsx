          ...
          <CardContent className="flex flex-col gap-4 border-t pt-6">
            <Alert>
              ...
              <AlertDescription>
                ...
                Формат — PDF до 10 МБ. Активный гайд уходит в обычной ветке
                «Получить гайд»; кампании по кодовому слову выбирают файл из
                этой же библиотеки отдельно.
              </AlertDescription>
            </Alert>
            {guides.length === 0 ? (
...
export default function BotScenarioPage() {
    ...
    <div className="flex max-w-6xl flex-col gap-6">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
            Что бот говорит клиенту в Telegram и MAX, какие материалы выдаёт и
            по каким кодовым словам. Изменения подхватываются в течение минуты —
            перезапуск не нужен.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Слева — шаги, сгруппированные по смыслу. Пустое поле означает
            «оставить текст по умолчанию», подсказка под полем показывает, когда
            сообщение отправляется.
          </p>
        </div>
        <BotTextsForm
          sections={defsData?.sections ?? []}
          groups={groups}
