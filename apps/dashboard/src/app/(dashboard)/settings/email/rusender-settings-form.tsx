...
type RusenderSettings = RouterOutputs["email"]["getRusenderSettings"];

function toFormValues(settings: RusenderSettings): RusenderSettingsInput {
	return {
...
export function RusenderSettingsForm() {
	...
	const form = useForm<RusenderSettingsInput>({
		resolver: zodResolver(rusenderSettingsSchema),
	...
	const mutation = useMutation(
		orpc.email.upsertRusenderSettings.mutationOptions({
			onSuccess: () => {
				toast.success("Настройки Rusender сохранены");
				form.reset(form.getValues());
				queryClient.invalidateQueries({
					queryKey: orpc.email.getRusenderSettings.key(),
				});
			},
			onError: (error) => {
