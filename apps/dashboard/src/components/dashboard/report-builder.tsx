...
import { bucketBy, computeGroupStats, summarize } from "@/lib/analytics/aggregate";
...
function dimensionLabel(key: string, dimension: DimensionId, dicts: ReportDictionaries): string {
  if (dimension === "source") return dicts.sources[key] ?? key;
  if (dimension === "category") return dicts.categories[key] ?? `Воронка ${key}`;
...
const METRICS: Array<{ id: MetricId; label: string; money?: boolean; percent?: boolean }> = [
...
function metricValue(row: GroupStats, metric: MetricId): number {
  if (metric === "conversionRate") return Number((row.conversionRate * 100).toFixed(1));
  return row[metric];
}

type FilterId = "status" | "category" | "source" | "utmSource" | "utmMedium" | "utmCampaign";

const FILTERS: Array<{ id: FilterId; label: string; valueOf: (deal: DealRecord) => string }> = [
...
function filterValueLabel(filter: FilterId, value: string, dicts: ReportDictionaries): string {
  if (filter === "status") return STATUS_LABELS[value as DealStatus] ?? value;
  if (filter === "category") return dicts.categories[value] ?? `Воронка ${value}`;
...
export function ReportBuilder({ deals, dictionaries }: { deals: DealRecord[]; dictionaries: ReportDictionaries }) {
  ...
  const filterOptions = useMemo(() => {
    const options = new Map<FilterId, Array<{ value: string; label: string; count: number }>>();
    for (const filter of FILTERS) {
      ...
      options.set(
        filter.id,
        [...counts.entries()]
          .map(([value, count]) => ({ value, label: filterValueLabel(filter.id, value, dictionaries), count }))
    ...
    () =>
      deals.filter((deal) =>
        FILTERS.every((filter) => {
          ...
          return selected.length === 0 || selected.includes(filter.valueOf(deal));
        }),
      ),
    [deals, filters],
  );
  ...
  const tableRows = useMemo(() => {
    ...
    return [...groups].sort((a, b) => {
      const av = a[key];
      const bv = b[key];
      const cmp = typeof av === "string" ? av.localeCompare(bv as string, "ru") : av - (bv as number);
      return desc ? -cmp : cmp;
    });
  }, [groups, sort, isTime]);
  ...
  const chartRows = useMemo(() => {
    const rows = isTime
      ...
      : [...groups].sort((a, b) => metricValue(b, metric) - metricValue(a, metric)).slice(0, CHART_TOP_LIMIT);
    return rows.map((row) => ({ label: row.label, value: metricValue(row, metric) }));
  }, [groups, isTime, metric]);

  const chartConfig = { value: { label: metricInfo.label, color: "var(--chart-1)" } } satisfies ChartConfig;
  const activeFilterCount = FILTERS.reduce((sum, f) => sum + (filters[f.id].length > 0 ? 1 : 0), 0);

  function toggleFilter(id: FilterId, value: string, checked: boolean) {
    setFilters((prev) => ({
      ...prev,
      [id]: checked ? [...prev[id], value] : prev[id].filter((v) => v !== value),
    }));
  }
  ...
  function SortHead({ label, sortKey, numeric }: { label: string; sortKey: SortKey; numeric?: boolean }) {
    return (
    ...
    <div className="flex flex-col gap-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Выберите группировку, метрику и фильтры — таблица и график обновятся сразу. Период задаётся в шапке.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-4">
          <div className="flex flex-wrap items-end gap-3">
            <div className="flex flex-col gap-1">
              ...
              <Select value={dimension} onValueChange={(v) => setDimension(v as DimensionId)}>
            ...
            <div className="flex flex-col gap-1">
              <Label className="text-xs text-muted-foreground">Метрика графика</Label>
            ...
            <div className="flex flex-col gap-1">
              <Label className="text-xs text-muted-foreground">Вид графика</Label>
              <Select value={chartKind} onValueChange={(v) => setChartKind(v as "bar" | "line")}>
                  ...
                  <DropdownMenuContent align="start" className="max-h-72 overflow-y-auto">
                    {(filterOptions.get(filter.id) ?? []).map((option) => (
                      <DropdownMenuCheckboxItem
                        ...
                        onCheckedChange={(checked) => toggleFilter(filter.id, option.value, checked === true)}
                        onSelect={(event) => event.preventDefault()}
                      >
                        {option.label}
                        <span className="ml-1 text-muted-foreground">({formatNumber(option.count)})</span>
                      </DropdownMenuCheckboxItem>
                    ))}
                  </DropdownMenuContent>
                </DropdownMenu>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {formatNumber(summary.totalDeals)} сделок · выиграно {formatNumber(summary.wonDeals)} · конверсия{" "}
            {formatPercent(summary.conversionRate)} · сумма выигранных {formatMoney(summary.wonSum)}
            {!isTime && groups.length > CHART_TOP_LIMIT && ` · на графике топ-${CHART_TOP_LIMIT} групп`}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {chartRows.length === 0 ? (
            <p className="py-8 text-center text-sm text-muted-foreground">Нет данных под выбранные фильтры</p>
          ) : (
            <ChartContainer config={chartConfig} className="h-[320px] w-full">
              {chartKind === "line" ? (
                ...
                <BarChart data={chartRows} layout="vertical" margin={{ left: 16 }}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {formatNumber(groups.length)} групп — сортировка по клику на заголовок столбца
          </CardDescription>
          <CardAction>
            <ExportCsvButton
              filename={`report-${dimension}.csv`}
              headers={[dimensionInfo.label, "Сделок", "Выиграно", "Конверсия, %", "Сумма выигранных", "Сумма в воронке"]}
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <SortHead label="Сумма в воронке" sortKey="opportunitySum" numeric />
              </TableRow>
            </TableHeader>
            <TableBody>
              {tableRows.length === 0 ? (
                ...
                tableRows.map((row) => (
                  <TableRow key={row.key}>
                    ...
                    <TableCell className="text-right tabular-nums">{formatMoney(row.opportunitySum)}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
