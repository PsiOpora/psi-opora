 ...
 */
export const bookPreorderOrders = pgTable(
	"book_preorder_orders",
	{
	...
	(table) => [
		// Часовой крон Б1–Б6 фильтрует по status='reserved' на каждом запуске
		// (listReservedBookPreorderOrders) — без индекса это последовательное
		// сканирование, которое линейно замедляется с ростом таблицы.
		index("book_preorder_orders_status_idx").on(table.status),
	],
);
