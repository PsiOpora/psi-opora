...
const fields = fieldsResp.result as Record<string, { type: string; statusType?: string; title: string; items?: unknown[] }>;
...
const out = {
  ...
  directoriesByEntity: Object.fromEntries(
    Object.entries(byEntity).map(([k, v]) => [
      k,
      v.map((r) => ({ STATUS_ID: r.STATUS_ID, NAME: r.NAME, SYSTEM: r.SYSTEM })),
    ]),
  ),
