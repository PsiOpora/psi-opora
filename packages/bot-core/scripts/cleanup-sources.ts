...
async function call(method: string, body: unknown) {
	...
	if (json.error) {
		throw new Error(
			`${method}: ${json.error} — ${json.error_description ?? ""}`,
		);
	}
	return json;
}
...
async function updateDealsSource(fromSourceId: string, toSourceId: string) {
	...
	console.log(
		`${fromSourceId} → ${toSourceId}: найдено ${deals.length} сделок`,
	);
...
function findRow(statusId: string) {
	...
	if (!row)
		throw new Error(`не найден статус ${statusId} в справочнике SOURCE`);
	return row;
}
