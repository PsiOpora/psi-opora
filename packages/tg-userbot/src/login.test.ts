...
const { confirmLoginCode, confirmLoginPassword, sendLoginCode } = await import("./login");
...
describe("confirmLoginCode", () => {
  test("успешный вход возвращает connected + сессию", async () => {
    const result = await confirmLoginCode(
      { pendingSession: "p", phone: "+7", phoneCodeHash: "hash-1", code: "12345" },
      CREDENTIALS,
    );
  ...
  test("прочие ошибки signIn пробрасываются наружу как есть", async () => {
    ...
    await expect(
      confirmLoginCode(
        { pendingSession: "p", phone: "+7", phoneCodeHash: "hash-1", code: "00000" },
        CREDENTIALS,
      ),
