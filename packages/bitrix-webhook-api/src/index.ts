...
export interface BitrixWebhookPayload {
	...
	data?: {
		...
		MESSAGES?: Array<{
			...
			message?: {
				/** Внешний ID сообщения, который коннектор передал в
				 * imconnector.send.messages. Для сообщений из единого инбокса
				 * начинается с `operator-` — по нему отсекаем собственное эхо. */
				id?: number | string;
				text?: string;
...
export interface OperatorReplyMessage {
	...
	text: string;
	/** Внешний ID из data.MESSAGES[].message.id. */
	externalMessageId?: number | string;
	/** Внутренний ID сообщения Bitrix из data.MESSAGES[].im.message_id —
	 * используем как ключ идемпотентности при повторной доставке вебхука. */
...
export function getOperatorReplyMessage(
	...
	return {
		...
		chatId,
		text,
		externalMessageId: item?.message?.id,
		bitrixMessageId: item?.im?.message_id,
...
export function bitrixWebhookHandler(options?: {
	...
	onConnectorDisabled?: (info: ConnectorDisabledInfo) => void | Promise<void>;
}) {
